...
class TestSectionOrderFeed:
    ...
    def test_open_modal_window_after_click_on_order(self, driver): # Работает!
        personal_account = PersonalAccountPage(driver, self.wait)
        open_modal_window = OrderFeedPage(driver, self.wait)
    ...
    def test_orders_display_in_order_feed(self, driver): # Работает!
        display_order = OrderFeedPage(driver, self.wait)
        personal_account = PersonalAccountPage(driver, self.wait)
        main_func = MainFunctionalityPage(driver, self.wait)
        personal_account.login()
    ...
    def test_total_order_count_increases_after_new_order(self, driver): # Работает!
        display_order = OrderFeedPage(driver, self.wait)
        personal_account = PersonalAccountPage(driver, self.wait)
        main_func = MainFunctionalityPage(driver, self.wait)
        personal_account.login()
    ...
    def test_daily_order_count_increases_after_new_order(self, driver): # Работает!
        display_order = OrderFeedPage(driver, self.wait)
        personal_account = PersonalAccountPage(driver, self.wait)
        main_func = MainFunctionalityPage(driver, self.wait)
        personal_account.login()
    ...
    def test_order_id_display_in_progress_section(self, driver): # Работает!!!
        display_order = OrderFeedPage(driver, self.wait)
        personal_account = PersonalAccountPage(driver, self.wait)
        main_func = MainFunctionalityPage(driver, self.wait)
        personal_account.login()
